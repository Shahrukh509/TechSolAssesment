...
class BookingRepository extends BaseRepository
{
    ...
    protected $model,$mailer,$logger,$user,$job,$userMeta,$userLanguage,$translator,$language;

    /**
     * @param Job $model
     */
    function __construct(Job $model, MailerInterface $mailer,User $user,Job $job,UserMeta $userMeta,UserLanguages $userLanguage,Translator $translator,UserLanguages $language)
    {
    ...
    public function getUsersJobs($user_id)
    {
        ...
        try{
            ...
            if(!$cuser) throw new Exception('no user found');
            $usertype = '';
            $emergencyJobs = array();
            $noramlJobs = array();
        ...
        if ($cuser->is('customer')) {
            ...
            $jobs = $this->job->getTranslatorJobs($cuser->id, 'new');
        ...
        if($jobs){
            ...
            ->each(function ($item, $key) use ($user_id){
                $item['usercheck'] = $this->job->checkParticularJob($user_id, $item);
            })
            ->sortBy('due')
            ->all();
        }
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        try{
            ...
            $cuser = $this->user->find($user_id);
           if(!$cuser) throw new Exception('no user found');$usertype = '';
           $emergencyJobs = array();
           $noramlJobs = array();
         if ($cuser->is('customer')){
             $jobs = $cuser->jobs() 
             ...
             $jobs_ids = $this->job->getTranslatorJobsHistoric($cuser->id, 'historic', $pagenum);
    ...
    public function storeJobEmail($data)
    {
        ...
        try{
            ...
            $job = $this->job->findOrFail($data['user_email_job_id']);
    ...
    public function jobToData($job)
    {
        ...
        try{
            ...
            if ($job->certified != null) {
                ...
                else if ($job->certified == 'yes') $data['job_for'][] = 'Auktoriserad';
                else if ($job->certified == 'n_health') $data['job_for'][] = 'Sjukvårdstolk';
                else if ($job->certified == 'law' || $job->certified == 'n_law') $data['job_for'][] = 'Rätttstolk';
                else $data['job_for'][] = $job->certified;
    ...
    public function jobEnd($post_data = array())
    {
        try{
            ...
            $job_detail = $this->job->with('translatorJobRel')->find($jobid);
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        try{
            ...
            $user_meta =  $this->userMeta->where('user_id', $user_id)->first();
            ...
            $languages = $userLanguage->where('user_id', '=', $user_id)->get();
            $userlanguage = collect($languages)->pluck('lang_id')->all();
            $gender = $user_meta->gender;
            $translator_level = $user_meta->translator_level;
            $job_ids = $this->job->getJobs($user_id, $job_type, 'pending', $userlanguage, $gender, $translator_level);
            foreach ($job_ids as $k => $v)     // checking translator town
            {
                $job = $this->job->find($v->id);
                $jobuserid = $job->user_id;
                $checktown = $this->job->checkTowns($jobuserid, $user_id);
                if (($job->customer_phone_type == 'no' || $job->customer_phone_type == '') && $job->customer_physical_type == 'yes' && $checktown == false) unset($job_ids[$k]);
                ...
                if ($oneUser->user_type == '2' && $oneUser->status == '1' && $oneUser->id != $exclude_user_id) { // user is translator and he is not disabled
                    ...
                    foreach ($jobs as $oneJob) {
                        if ($job->id == $oneJob->id) { // one potential job is the same with current job
                            $userId = $oneUser->id;
                            $job_for_translator = $this->job->assignedToPaticularTranslator($userId, $oneJob->id);
                            if ($job_for_translator == 'SpecificJob') {
                                $job_checker = $this->job->checkParticularJob($userId, $oneJob);
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        try{
            ...
            $jobPosterMeta =  $this->userMeta->where('user_id', $job->user_id)->first();
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        try{
            ...
            if (!empty($job->certified)) {
                ...
                elseif($job->certified == 'law' || $job->certified == 'n_law') $translator_level[] = 'Certified with specialisation in law';
                elseif($job->certified == 'health' || $job->certified == 'n_health') $translator_level[] = 'Certified with specialisation in health care';
    ...
    private function changeStatus($job, $data, $changedTranslator)
    {
        ...
        try{
            //  this is what we call factory design pattern
...
//      
    ...
    private function changePendingStatus($job, $data, $changedTranslator)
    {
        try{
            ...
            if ($data['status'] == 'assigned' && $changedTranslator) {
                ...
                $translator = $this->job->getJobsAssignedTranslatorDetail($job);
        ...
        try{
            ...
            if (!is_null($current_translator) || (isset($data['translator']) && $data['translator'] != 0) || $data['translator_email'] != '') {
                ...
                if (!is_null($current_translator) && ((isset($data['translator']) && $current_translator->user_id != $data['translator']) || $data['translator_email'] != '') && (isset($data['translator']) && $data['translator'] != 0)) {
                    ...
                    $new_translator = $this->translator->create($new_translator);
                    ...
                    if ($data['translator_email'] != '') $data['translator'] =$this->user->where('email', $data['translator_email'])->first()->id;
                    $new_translator = $this->translator->create(['user_id' => $data['translator'], 'job_id' => $job->id]);
                    $log_data[] = [
                        'old_translator' => null,
                        'new_translator' => $new_translator->user->email
                    ];
    ...
    public function sendNotificationByAdminCancelJob($job_id)
    {
        ...
        try{
            ...
            $job = $this->job->findOrFail($job_id);
    ...
    public function acceptJob($data, $user)
    {
        ...
        try{
            ...
            $job_id = $data['job_id'];
            $job = $this->job->findOrFail($job_id);
            if (!$this->job->isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due)) {
                if ($job->status == 'pending' && $this->job->insertTranslatorJobRel($cuser->id, $job_id)) {
                    $job->status = 'assigned';
                    $job->save();
                    $user = $job->user()->first();
                    $mailer = new AppMailer();
            ...
            */
            ...
            if ($cuser->is('customer')) {
                ...
                if ($job->due->diffInHours(Carbon::now()) > 24) {
                    ...
                    $job->save();
    //                Event::fire(new JobWasCanceled($job));
                    $this->job->deleteTranslatorJobRel($translator->id, $job_id);
    ...
    public function getPotentialJobs($cuser)
    {
        ...
        $userlanguage = $userLanguage->where('user_id', '=', $cuser->id)->pluck('lang_id');
        ...
        $job_ids = $this->job->getJobs($cuser->id, $job_type, 'pending', $userlanguage, $gender, $translator_level);
        foreach ($job_ids as $k => $job) {
            $jobuserid = $job->user_id;
            $job->specific_job = $this->job->assignedToPaticularTranslator($cuser->id, $job->id);
            $job->check_particular_job = $this->job->checkParticularJob($cuser->id, $job);
            $checktown = $this->job->checkTowns($jobuserid, $cuser->id);
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
        ...
        try{
            ...
            foreach ($jobs as $job) {
                ...
                if (count($sessionTime) >= 3) {
                    ...
                    if (($diff[$i] >= $job->duration) && ($diff[$i] >= $job->duration * 2)) $sesJobs[$i] = $job;
                    $i++;
                }
            }
            foreach ($sesJobs as $job) $jobId [] = $job->id;
            $languages = $this->language->where('active', '1')->orderBy('language')->get();
            $requestdata = Request::all();
    ...
    {
        try{
            $throttles = Throttles::whereIgnore(0)->with('user')->paginate(15);
    ...
    public function bookingExpireNoAccepted()
    {
        try{
            ...
            $languages = $this->language->whereActive('1')->orderBy('language')->get();
        $requestdata = Request::all();
        $all_customers = DB::table('users')->whereUserType('user_type', '1')->lists('email');
        $all_translators = DB::table('users')->where('user_type', '2')->lists('email');
        $cuser = Auth::user();
        $consumer_type = TeHelper::getUsermeta($cuser->id, 'consumer_type');
    ...
    //        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
            ...
            if ($job['status'] != 'timedout') {
                $affectedRows = $this->job->where('id', '=', $jobid)->update($datareopen);
                $new_jobid = $jobid;
            } else {
                $job['status'] = 'pending';
                ...
                $affectedRows = $this->job->create($job);
                $new_jobid = $affectedRows['id'];
            }
            //$result = DB::table('translator_job_rel')->insertGetId($data);
            $this->translator->where('job_id', $jobid)->where('cancel_at', NULL)->update(['cancel_at' => $data['cancel_at']]);
            $Translator = $this->translator->create($data);
